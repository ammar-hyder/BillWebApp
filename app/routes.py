...
def fetch_disco(disco_name):
    try:
        ...
        return render_template(
            ...
            result_summary=f"Processed {disco_name} bills: {len(results_df)} records.",
            error=None
        )
    except Exception as e:
        return render_template(
...
def fetch_all():
    try:
        ...
        # Optional: clean downloads each run (uncomment if desired)
        ...
        # Write combined Excel results
        excel_path = os.path.join(downloads_dir, "results_ALL.xlsx")
        with pd.ExcelWriter(excel_path, engine='xlsxwriter') as writer:
            start_row = 0
            if not df_others_results.empty:
                df_others_results.to_excel(writer, sheet_name='Results', startrow=start_row, index=False)
                start_row += len(df_others_results) + 2
            if not df_ke_results.empty:
                df_ke_results.to_excel(writer, sheet_name='Results', startrow=start_row, index=False)
        ...
        # Read Excel to display in table
        combined_df = pd.read_excel(excel_path)
        ...
        return render_template(
            ...
            result_summary=f"Processed ALL DISCO bills: {len(combined_df)} records.",
            error=None
        )
